...
例如定义一个变量，变量名称为name，值为juzicoding，同时打印name这个变量。
...
Python 的整数没有大小限制，想写多长写多长；浮点数超出一定范围会直接显示为 `inf`（无限大）。
...
# 我的名字是橘子coding, 橘子coding网址是juzicoding.com
print('我的名字是{0}, {0}的网址是{1}'.format('橘子coding', 'juzicoding.com'))
# 我的名字是橘子coding, 网站地址是juzicoding.com
...
`list.extend(iterable)` 往列表当中添加多个元素，参数为=列表，同样为末尾追加。
...
当然，我只列举的部分常用的，学无止境，还有很多方法可以自己研究~
...
不过 Unicode 通常用 2 个字节表示一个字符，如果文本基本都是英文，存储和传输要浪费一倍空间。所以又出现了「可变长编码」UTF-8：英文字母编码成 1 个字节，汉字通常是 3 个字节，只有生僻字才需要 4-6 个字节。UTF-8 兼容 ASCII，历史软件可以直接工作。
