...
# 缩放到原来的 50%（thumbnail 保持宽高比，实际尺寸不超过目标框）
im.thumbnail((w // 2, h // 2))
print(f'缩放后尺寸: {im.size}')
...
如果运行时报 `OSError: cannot open resource`（旧版本也可能显示为 `IOError`），说明 Pillow 找不到字体文件，把 `Arial.ttf` 换成绝对路径，比如 macOS 下的 `/Library/Fonts/Arial.ttf` 或 `/System/Library/Fonts/Supplemental/Arial.ttf` 即可。
...
```python
# 读取响应中的 Cookie（具体 Cookie 名以网站实际返回为准）
r = requests.get('https://www.douban.com/')
print(r.cookies.get('ts'))
