...
> 提示：`datetime.utcfromtimestamp()` 和 `datetime.utcnow()` 在 Python 3.12 起已弃用，推荐改用 `datetime.fromtimestamp(ts, tz=timezone.utc)` 等方式。
...
> 上面用到了 `datetime.utcnow()`，它在 Python 3.12 起已弃用，建议换成 `datetime.now(timezone.utc)`。
...
按插入顺序保存 key 的字典。Python 3.7 起普通 `dict` 也已保证插入顺序，`OrderedDict` 的独特价值更多体现在 `move_to_end()` 等方法及顺序敏感的相等比较上。
...
Base64 不是加密，只是一种编码。标准 Base64 的字符串长度必须是 4 的倍数，不够时末尾会用 `=` 补齐；很多场景（比如 URL）会把末尾的 `=` 去掉，解码时再补回来即可：
...
# b'aGVsbG8'，去掉末尾的 =
print(base64.b64encode(b'hello').rstrip(b'='))
```
