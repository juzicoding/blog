...
> 在文本模式下，`whence` 通常只能使用 `0`（相对文件开头），且偏移量建议来自 `tell()` 的返回值；只有 `seek(0, 2)` 跳到文件尾是例外。二进制模式下可以使用 `1` 和 `2`。
...
把内存中的对象保存到文件，或通过网络传输，需要先把对象转成字节序列，这个过程叫**序列化**；反过来的过程叫**反序列化**。
...
print(obj)  # {'name': 'juzicoding', 'age': 18}
```

`json.dump()` 和 `json.load()` 可以直接操作文件对象。
...
> 跨语言共享数据优先用 JSON；只在 Python 内部使用时才考虑 pickle。
...
- StringIO/BytesIO 可以在内存中像文件一样操作字符串和字节。
