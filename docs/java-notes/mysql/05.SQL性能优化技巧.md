...
![尽量全值匹配](https://juzicoding.com/img/2025/01KDAEV95B3RR1VYV2NE6CD7E0.webp)
...
![最左前缀原则1](https://juzicoding.com/img/2025/01KDAEVB381GNG2XANBYBV3YS2.webp)
...
![最左前缀原则2](https://juzicoding.com/img/2025/01KDAEVCYT8BEHGP976VH91VRJ.webp)

可以看到即使颠倒了顺序，三个的执行计划也是一毛一样的。

### 1.3 不在索引列上做任何操作(计算、函数、(自动/手动)类型转换)，会导致索引失效而转向全表扫描
...
![索引列不做任何计算、函数、(自动/手动)类型转换)操作](https://juzicoding.com/img/2025/01KDAEVETMG3JCV6R85P0NAW1K.webp)
...
![存储引擎不能使用索引中范围条件右边的列](https://juzicoding.com/img/2025/01KDAEVGNTR8ZAZKZ9TV8V0779.webp)

可以看到第二条 SQL 使用了 `name,age` 列作为索引来查询，`position` 并没有使用。

### 1.5 尽量使用覆盖索引，减少 `select *` 语句

覆盖索引前面的文章提到过，不赘述了。

### 1.6 MySQL 在使用不等于( != 或者 <> )，not in，not exists 的时候无法使用索引会导致全表扫描
...
![范围查找要合理](https://juzicoding.com/img/2025/01KDAEVJHGAFADKAD79ZPP53MH.webp)

### 1.7 is null 和 is not null 一般情况下也无法使用索引
...
![isnull和isnotnull无法使用索引](https://juzicoding.com/img/2025/01KDAEVMC1KXV3KW16AKGB49FN.webp)

### 1.8 like 以通配符开头('%itwxe...') MySQL 索引失效会变成全表扫描操作
...
![like通配符开头索引失效](https://juzicoding.com/img/2025/01KDAEVP66DHXSVZRPVJM9GJTJ.webp)
...
![image-20220213162216965](https://juzicoding.com/img/2025/01KDAEVR2G43W7E1GYQT91HFWD.webp)
...
### 1.9 类型不匹配 MySQL 自动转型导致索引失效

字符串不加单引号索引失效，亦或是数值类型加单引号索引失效，也就是第三点中提到的自动类型转换(也叫隐式转换)导致索引失效。

### 1.10 少用 or 或 in ，用它查询时，MySQL 不一定使用索引
...
![非主键字段排序的分页查询](https://juzicoding.com/img/2025/01KDAEVSYK7T6TSCMDNABH2T1V.webp)

可以看到查询相同的结果，但是非常大的查询速度差距，这还只是10w+的测试数据，加入是100w呢，差距得多大嘞！

![阿里手册分页规约](https://juzicoding.com/img/2025/01KDAEVVTAZNRGJ73VM0FDQ89X.webp)

阿里Java开发手册中也对这种情况进行了说明，非常推荐各位小伙伴们看下阿里Java开发手册，其中专门有一个章节对 MySQL 开发规范进行了说明。

![阿里Java开发手册-MySQL数据库](https://juzicoding.com/img/2025/01KDAEVXY7WA1GK89Q6YXQ6ZGS.webp)
...
![NLJ算法](https://juzicoding.com/img/2025/01KDAEVZSDM685SEWZKRNNSS48.webp)
...
![BNL算法](https://juzicoding.com/img/2025/01KDAEW1M6A2DSG27A2SK06GWV.webp)
...
![count优化](https://juzicoding.com/img/2025/01KDAEW3M4S29Q3A7PEAM22JXT.webp)
...
![查询MySQL自己维护的总行数](https://juzicoding.com/img/2025/01KDAEW5MBGJG0JG8REDFESWPR.webp)
...
![show table status](https://juzicoding.com/img/2025/01KDAEW7MTPA5J1PNP7K6XXXF0.webp)
