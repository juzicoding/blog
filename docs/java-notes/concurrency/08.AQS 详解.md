...
- 通过 `signal` 或 `signalAll` 将等待队列中的节点转移到同步队列。（由等待队列转化为同步队列）
...
- 调用 `await` 方法阻塞线程；
- 当前线程存在于同步队列的头结点时，调用 `await` 方法进行阻塞，会从同步队列转化到等待队列。
...
在挂起线程前，需确认当前节点的上一个节点的状态。若为 1，代表是已取消的节点，不能挂起；若为 -1，代表后续节点中有需要挂起的线程，可以放心挂起；若为 -2 (线程在等待队列中) 或 -3 (避免共享锁线程无法唤醒的一个状态)，需要将状态改为 -1 之后，才能挂起当前线程。
