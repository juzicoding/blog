...
### 1.1 什么是 ThreadLocal？
根据 Java 官方文档的描述，`ThreadLocal` 类用于提供**线程内部的局部变量**。这些变量在多线程环境下访问（通过 `get` 和 `set` 方法）时，能保证各个线程的变量相对独立于其他线程内的变量。`ThreadLocal` 实例通常被声明为 `private static` 类型，用于关联线程和线程上下文。
