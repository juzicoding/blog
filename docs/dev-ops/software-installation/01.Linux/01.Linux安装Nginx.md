...
- Nginx版本：nginx-1.30.1

**下载安装文件**

采用源码包方式安装，进入目录(个人习惯/juzi/soft)，下载安装文件，如果云服务器下载速度过慢也可本地下载后上传。
...
``` bash
./configure --prefix=/juzi/soft/nginx \
...
```bash
/juzi/soft/nginx/sbin/nginx -c /juzi/soft/nginx/conf/nginx.conf
```
...
ExecStart=/juzi/soft/nginx/sbin/nginx -c /juzi/soft/nginx/conf/nginx.conf
